# 1 "/data/isivkov/libdbcsr_svn18247/src/dbcsr/mm/dbcsr_mm_cannon.F"
!--------------------------------------------------------------------------------------------------!
      ...
      grouped_k_index: DO metronome = 1, nsteps_k
         ...
         DO v_ki = 0, min_nimages-1
            ...
            CALL dbcsr_reset_vlocals(left_buffer_calc%mats(1, v_ki+1), &
                                     left_set%image_dist)
            !
            CALL ensure_array_size(k_sizes, ub=array_size(right_buffer_calc%mats(v_ki+1, 1)%local_rows))
            CALL local_filter(array_data(right_buffer_calc%mats(v_ki+1, 1)%row_blk_size), &
                              array_size(right_buffer_calc%mats(v_ki+1, 1)%local_rows), &
                              array_data(right_buffer_calc%mats(v_ki+1, 1)%local_rows), &
                              k_sizes)
            !
            IF (has_acc) THEN
               CALL dbcsr_data_host2dev(left_buffer_calc%mats(1, v_ki+1)%data_area)
               CALL dbcsr_data_host2dev(right_buffer_calc%mats(v_ki+1, 1)%data_area)
...
!$OMP BARRIER
            ...
            CALL dbcsr_mm_multrec_multiply(multrec(ithread)%p, &
                                           left=left_buffer_calc%mats(1, v_ki+1), &
                                           right=right_buffer_calc%mats(v_ki+1, 1), &
                                           flop=flop_single, &
...
# 1 "/data/isivkov/libdbcsr_svn18247/src/dbcsr/mm/dbcsr_mm_cannon.f90" 1
...
# 2 "/data/isivkov/libdbcsr_svn18247/src/dbcsr/mm/dbcsr_mm_cannon.f90" 2
# 3 "/data/isivkov/libdbcsr_svn18247/src/dbcsr/mm/dbcsr_mm_cannon.f90"
...
! **************************************************************************************************
  SUBROUTINE prepare_buffers_d(negate_real, negate_imaginary, &
    ...
    REAL(kind=real_8), DIMENSION(:), INTENT(IN)                     :: data_area
...
# 83 "/data/isivkov/libdbcsr_svn18247/src/dbcsr/mm/dbcsr_mm_cannon.f90"
                send_data_area%d%r_dp(sd_pos:myt_sdp(dst_p)-1) = &
                     -send_data_area%d%r_dp(sd_pos:myt_sdp(dst_p)-1)
# 92 "/data/isivkov/libdbcsr_svn18247/src/dbcsr/mm/dbcsr_mm_cannon.f90"
             ELSEIF (negate_imaginary) THEN
# 97 "/data/isivkov/libdbcsr_svn18247/src/dbcsr/mm/dbcsr_mm_cannon.f90"
             ENDIF
          ELSE
             ! Copy the block
             IF (PRESENT(scale_value)) THEN
...
! **************************************************************************************************
  SUBROUTINE prepare_buffers_s(negate_real, negate_imaginary, &
    ...
    INTEGER, DIMENSION(:, :), INTENT(IN), POINTER           :: blacs2mpi
    CHARACTER, INTENT(IN)                                   :: predist_type_fwd
    REAL(kind=real_4), DIMENSION(:), INTENT(IN)                     :: data_area
    TYPE(dbcsr_data_obj), INTENT(INOUT)                     :: send_data_area
    TYPE(dbcsr_scalar_type), INTENT(IN)                     :: scale_neg_one
    TYPE(dbcsr_scalar_type), INTENT(IN), OPTIONAL           :: scale_value
...
! **************************************************************************************************
  SUBROUTINE prepare_buffers_z(negate_real, negate_imaginary, &
    ...
    INTEGER, DIMENSION(:, :), INTENT(IN), POINTER           :: blacs2mpi
    CHARACTER, INTENT(IN)                                   :: predist_type_fwd
    COMPLEX(kind=real_8), DIMENSION(:), INTENT(IN)                     :: data_area
    TYPE(dbcsr_data_obj), INTENT(INOUT)                     :: send_data_area
    TYPE(dbcsr_scalar_type), INTENT(IN)                     :: scale_neg_one
    TYPE(dbcsr_scalar_type), INTENT(IN), OPTIONAL           :: scale_value
    ...
    DO WHILE (dbcsr_iterator_blocks_left(iter))
       ...
       DO symmetry_i = 1, nsymmetries
          ...
          IF (tr) THEN
             ...
             ELSEIF (negate_real) THEN
# 86 "/data/isivkov/libdbcsr_svn18247/src/dbcsr/mm/dbcsr_mm_cannon.f90"
                send_data_area%d%c_dp(sd_pos:myt_sdp(dst_p)-1) = &
                     CMPLX( &
...
# 92 "/data/isivkov/libdbcsr_svn18247/src/dbcsr/mm/dbcsr_mm_cannon.f90"
             ELSEIF (negate_imaginary) THEN
# 94 "/data/isivkov/libdbcsr_svn18247/src/dbcsr/mm/dbcsr_mm_cannon.f90"
                send_data_area%d%c_dp(sd_pos:myt_sdp(dst_p)-1) = &
...
! **************************************************************************************************
  SUBROUTINE prepare_buffers_c(negate_real, negate_imaginary, &
    ...
    INTEGER, DIMENSION(:, :), INTENT(IN), POINTER           :: blacs2mpi
    CHARACTER, INTENT(IN)                                   :: predist_type_fwd
    COMPLEX(kind=real_4), DIMENSION(:), INTENT(IN)                     :: data_area
    TYPE(dbcsr_data_obj), INTENT(INOUT)                     :: send_data_area
    TYPE(dbcsr_scalar_type), INTENT(IN)                     :: scale_neg_one
    TYPE(dbcsr_scalar_type), INTENT(IN), OPTIONAL           :: scale_value
...
# 117 "/data/isivkov/libdbcsr_svn18247/src/dbcsr/mm/dbcsr_mm_cannon.f90"
# 2001 "/data/isivkov/libdbcsr_svn18247/src/dbcsr/mm/dbcsr_mm_cannon.F" 2
