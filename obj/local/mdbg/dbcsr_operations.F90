# 1 "/data/isivkov/libdbcsr_svn18247/src/dbcsr/ops/dbcsr_operations.F"
!--------------------------------------------------------------------------------------------------!
...
MODULE dbcsr_operations
   ...
   USE dbcsr_methods,                   ONLY: &
        ...
        dbcsr_nfullrows_local, dbcsr_get_num_blocks
      ...
      IF (has_limits) THEN
         IF (limits(last_col_i) .NE. 0) THEN
            IF (debug_mod .AND. (limits(last_col_i) < 0 .OR. limits(last_col_i) > dbcsr_nfullcols_total(matrix_a))) &
               CPABORT("Specified last column is out of bounds.")
         ...
         IF (limits(first_col_i) .NE. 0) THEN
            IF (debug_mod .AND. (limits(first_col_i) < 0 .OR. limits(first_col_i) > dbcsr_nfullcols_total(matrix_a))) &
               CPABORT("Specified first column is out of bounds.")
         ...
         IF (limits(last_row_i) .NE. 0) THEN
            IF (debug_mod .AND. (limits(last_row_i) < 0 .OR. limits(last_row_i) > dbcsr_nfullrows_total(matrix_a))) &
               CPABORT("Specified last row is out of bounds.")
         ...
         IF (limits(first_row_i) .NE. 0) THEN
            IF (debug_mod .AND. (limits(first_row_i) < 0 .OR. limits(first_row_i) > dbcsr_nfullrows_total(matrix_a))) &
               CPABORT("Specified first row is out of bounds.")
...
!   ---------------------------------------------------------------------------
      ...
      IF (data_type_a .NE. my_beta_scalar%data_type) &
         CPABORT("beta type parameter not consistent with matrices type")
      ...
      IF ((.NOT. dbcsr_scalar_are_equal(my_beta_scalar, &
         ...
         ! Overestimate for summetric matrix and multiple threads!
         local_matrix_size = dbcsr_nfullrows_local(matrix_a)* &
      ...
      ELSE
         ...
         IF (.NOT. shallow) THEN
            IF (matrix_a%nze > dbcsr_get_data_size(matrix_a)) &
               CPABORT("Source matrix sizes not consistent!")
...
!   ---------------------------------------------------------------------------
      ...
      IF (PRESENT(block_row_bounds)) THEN
         IF (SIZE(block_row_bounds) /= 2) &
            CPABORT("Size of bounds specifier must be 2")
      ENDIF
...
# 1 "/data/isivkov/libdbcsr_svn18247/src/dbcsr/ops/dbcsr_operations.f90" 1
!--------------------------------------------------------------------------------------------------!
!   CP2K: A general program to perform molecular dynamics simulations                              !
!   Copyright (C) 2000 - 2018  CP2K developers group                                               !
!--------------------------------------------------------------------------------------------------!

# 1 "/data/isivkov/libdbcsr_svn18247/src/dbcsr/ops/../data/dbcsr.fypp" 1
# 8 "/data/isivkov/libdbcsr_svn18247/src/dbcsr/ops/../data/dbcsr.fypp"
# 39 "/data/isivkov/libdbcsr_svn18247/src/dbcsr/ops/../data/dbcsr.fypp"
# 7 "/data/isivkov/libdbcsr_svn18247/src/dbcsr/ops/dbcsr_operations.f90" 2
# 8 "/data/isivkov/libdbcsr_svn18247/src/dbcsr/ops/dbcsr_operations.f90"
! **************************************************************************************************
   ...
   SUBROUTINE dbcsr_add_anytype_d(matrix_a, matrix_b, iter, iw, do_scale, &
     ...
     DO WHILE (dbcsr_iterator_blocks_left(iter))
        ...
        IF (found) THEN
           ...
           IF (do_scale) THEN
              CALL daxpy (nze, my_beta_scalar%r_dp,  &
                          matrix_b%data_area%d%r_dp(lb_b:lb_b+nze-1), 1, &
                          matrix_a%data_area%d%r_dp(lb_a:lb_a+nze-1), 1)
           ELSE
              matrix_a%data_area%d%r_dp(lb_a:lb_a+nze-1) = &
                   matrix_a%data_area%d%r_dp(lb_a:lb_a+nze-1) + &
                   matrix_b%data_area%d%r_dp(lb_b:lb_b+nze-1)
           ENDIF
        ...
        ELSE
           ...
           IF (do_scale) THEN
              matrix_a%wms(iw)%data_area%d%r_dp(lb_a:matrix_a%wms(iw)%datasize) = &
                   my_beta_scalar%r_dp*matrix_b%data_area%d%r_dp(lb_b:lb_b+nze-1)
           ELSE
              matrix_a%wms(iw)%data_area%d%r_dp(lb_a:matrix_a%wms(iw)%datasize) = &
                   matrix_b%data_area%d%r_dp(lb_b:lb_b+nze-1)
           ENDIF
...
! **************************************************************************************************
   ...
   SUBROUTINE dbcsr_add_anytype_s(matrix_a, matrix_b, iter, iw, do_scale, &
                                              my_beta_scalar, my_flop)
     TYPE(dbcsr_type), INTENT(INOUT)                         :: matrix_a
     TYPE(dbcsr_type), INTENT(IN)                            :: matrix_b
     TYPE(dbcsr_iterator), INTENT(INOUT)                     :: iter
     ...
     DO WHILE (dbcsr_iterator_blocks_left(iter))
        ...
        IF (found) THEN
           ...
           IF (do_scale) THEN
              CALL saxpy (nze, my_beta_scalar%r_sp,  &
                          matrix_b%data_area%d%r_sp(lb_b:lb_b+nze-1), 1, &
                          matrix_a%data_area%d%r_sp(lb_a:lb_a+nze-1), 1)
           ELSE
              matrix_a%data_area%d%r_sp(lb_a:lb_a+nze-1) = &
                   matrix_a%data_area%d%r_sp(lb_a:lb_a+nze-1) + &
                   matrix_b%data_area%d%r_sp(lb_b:lb_b+nze-1)
           ENDIF
           my_flop = my_flop + nze * 2
        ELSE
           ! let's sum the block in the wms
           ...
           matrix_a%wms(iw)%datasize = matrix_a%wms(iw)%datasize + nze
           IF (do_scale) THEN
              matrix_a%wms(iw)%data_area%d%r_sp(lb_a:matrix_a%wms(iw)%datasize) = &
                   my_beta_scalar%r_sp*matrix_b%data_area%d%r_sp(lb_b:lb_b+nze-1)
           ELSE
              matrix_a%wms(iw)%data_area%d%r_sp(lb_a:matrix_a%wms(iw)%datasize) = &
                   matrix_b%data_area%d%r_sp(lb_b:lb_b+nze-1)
           ENDIF
        ...
        ENDIF
     ENDDO
   END SUBROUTINE dbcsr_add_anytype_s
# 8 "/data/isivkov/libdbcsr_svn18247/src/dbcsr/ops/dbcsr_operations.f90"
! **************************************************************************************************
!> \brief traces a DBCSR matrix
!> \param[in] matrix_a       DBCSR matrix
...
! **************************************************************************************************
   ...
   SUBROUTINE dbcsr_add_anytype_z(matrix_a, matrix_b, iter, iw, do_scale, &
                                              my_beta_scalar, my_flop)
     TYPE(dbcsr_type), INTENT(INOUT)                         :: matrix_a
     TYPE(dbcsr_type), INTENT(IN)                            :: matrix_b
     TYPE(dbcsr_iterator), INTENT(INOUT)                     :: iter
     ...
     DO WHILE (dbcsr_iterator_blocks_left(iter))
        ...
        IF (found) THEN
           ...
           lb_a = ABS (matrix_a%blk_p(blk))
           IF (do_scale) THEN
              CALL zaxpy (nze, my_beta_scalar%c_dp,  &
                          matrix_b%data_area%d%c_dp(lb_b:lb_b+nze-1), 1, &
                          matrix_a%data_area%d%c_dp(lb_a:lb_a+nze-1), 1)
           ELSE
              matrix_a%data_area%d%c_dp(lb_a:lb_a+nze-1) = &
                   matrix_a%data_area%d%c_dp(lb_a:lb_a+nze-1) + &
                   matrix_b%data_area%d%c_dp(lb_b:lb_b+nze-1)
           ENDIF
           my_flop = my_flop + nze * 2
        ELSE
           ! let's sum the block in the wms
           ...
           matrix_a%wms(iw)%datasize = matrix_a%wms(iw)%datasize + nze
           IF (do_scale) THEN
              matrix_a%wms(iw)%data_area%d%c_dp(lb_a:matrix_a%wms(iw)%datasize) = &
                   my_beta_scalar%c_dp*matrix_b%data_area%d%c_dp(lb_b:lb_b+nze-1)
           ELSE
              matrix_a%wms(iw)%data_area%d%c_dp(lb_a:matrix_a%wms(iw)%datasize) = &
                   matrix_b%data_area%d%c_dp(lb_b:lb_b+nze-1)
           ENDIF
        ...
        ENDIF
     ENDDO
   END SUBROUTINE dbcsr_add_anytype_z
# 8 "/data/isivkov/libdbcsr_svn18247/src/dbcsr/ops/dbcsr_operations.f90"
! **************************************************************************************************
!> \brief traces a DBCSR matrix
!> \param[in] matrix_a       DBCSR matrix
...
! **************************************************************************************************
   ...
   SUBROUTINE dbcsr_add_anytype_c(matrix_a, matrix_b, iter, iw, do_scale, &
                                              my_beta_scalar, my_flop)
     TYPE(dbcsr_type), INTENT(INOUT)                         :: matrix_a
     TYPE(dbcsr_type), INTENT(IN)                            :: matrix_b
     TYPE(dbcsr_iterator), INTENT(INOUT)                     :: iter
     ...
     DO WHILE (dbcsr_iterator_blocks_left(iter))
        ...
        IF (found) THEN
           ...
           IF (do_scale) THEN
              CALL caxpy (nze, my_beta_scalar%c_sp,  &
                          matrix_b%data_area%d%c_sp(lb_b:lb_b+nze-1), 1, &
                          matrix_a%data_area%d%c_sp(lb_a:lb_a+nze-1), 1)
           ELSE
              matrix_a%data_area%d%c_sp(lb_a:lb_a+nze-1) = &
                   matrix_a%data_area%d%c_sp(lb_a:lb_a+nze-1) + &
                   matrix_b%data_area%d%c_sp(lb_b:lb_b+nze-1)
           ENDIF
           my_flop = my_flop + nze * 2
        ELSE
           ! let's sum the block in the wms
           ...
           matrix_a%wms(iw)%datasize = matrix_a%wms(iw)%datasize + nze
           IF (do_scale) THEN
              matrix_a%wms(iw)%data_area%d%c_sp(lb_a:matrix_a%wms(iw)%datasize) = &
                   my_beta_scalar%c_sp*matrix_b%data_area%d%c_sp(lb_b:lb_b+nze-1)
           ELSE
              matrix_a%wms(iw)%data_area%d%c_sp(lb_a:matrix_a%wms(iw)%datasize) = &
                   matrix_b%data_area%d%c_sp(lb_b:lb_b+nze-1)
           ENDIF
        ...
        ENDIF
     ENDDO
   END SUBROUTINE dbcsr_add_anytype_c
# 506 "/data/isivkov/libdbcsr_svn18247/src/dbcsr/ops/dbcsr_operations.f90"
# 2753 "/data/isivkov/libdbcsr_svn18247/src/dbcsr/ops/dbcsr_operations.F" 2
